...
class CADSegmentation(Dataset):
    ...
    def __init__(self, CAD_folder=None, list_txt=None):
   
        super().__init__()
        
        print('CAD Images from : {}'.format(CAD_folder))
        print('list txt is ',list_txt)
        ...
        if list_txt is not None:
            with open(os.path.join(list_txt), "r") as f:
    ...
    def __getitem__(self, index):
        _cad_img, _target_img, _img_name = self._make_img_gt_point_pair(index)
        sample = {'image': _cad_img, 'label': _target_img}
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        return _cad_img, _target, _img_name
